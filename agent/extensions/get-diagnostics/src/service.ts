...
const resolveFiles = async (
  ...
  try {
    ...
    log('service', 'resolveFiles: stat failed, treating as single file', {
      absPath,
    });
...
export const createDiagnosticsService = (
  ...
  const getVersion = (absPath: string): number =>
    fileVersions.get(absPath) ?? 0;
  ...
  const cacheStore = (
    ...
    if (getVersion(absPath) !== version) {
      log('service', 'cache: stale, discarding', {
        providerId,
        file: absPath,
        version,
      });
  ...
  const proactiveCheck = (
    ...
    log('service', 'proactive check: start', {
      provider: provider.id,
      file: absPath,
      version,
    });
  ...
  const getDiagnostics = async (
    ...
    const results = await Promise.allSettled(
      selectedProviders.map(async provider => {
        ...
        if (providerFiles.length === 0) {
          ...
          log(
            'service',
            `provider ${provider.id}: skipped (no supported files)`
          );
          return [];
        }
        ...
        const timeoutMs =
          userTimeoutMs ?? provider.defaultTimeoutMs ?? DEFAULT_TIMEOUT_MS;
    ...
    const diagnostics = truncated
      ? processed.slice(0, MAX_DIAGNOSTICS)
      : processed;
  ...
  const syncDocument = (filePath: string, content?: string): void => {
    const absPath = path.isAbsolute(filePath)
      ? filePath
      : path.resolve(serviceCwd ?? '', filePath);
    const version = bumpVersion(absPath);
    log('service', 'syncDocument', {
      file: absPath,
      version,
