...
export const formatDiagnosticsResult = (
  result: GetDiagnosticsResult
): string => {
  const { diagnostics, processedFiles, providerStatus, truncated, timingMs } =
    result;
  ...
  const sections = R.pipe(
    ...
    R.groupBy(d => d.path),
    grouped =>
      Object.entries(grouped).map(([path, diags]) =>
        formatFileSection(path, diags)
      )
  );
