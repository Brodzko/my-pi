...
const GetDiagnosticsParams = Type.Object({
  path: Type.String({
    description:
      'File or directory path to check. Relative to cwd or absolute.',
  }),
  ...
  maxFiles: Type.Optional(
    Type.Number({
      description:
        'Maximum files to check when path is a directory. Default: 200.',
    })
  ),
});

export const registerGetDiagnosticsTool = (
  pi: ExtensionAPI,
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      log('tool', 'execute: start', {
        params: {
          ...
          content: params.content
            ? `<${params.content.length} chars>`
            : undefined,
        },
      });

      // Normalize path — strip @ prefix if present (some agents prepend it)
      const targetPath = params.path.startsWith('@')
        ? params.path.slice(1)
        : params.path;
      ...
      log('tool', 'execute: resolved path', {
        targetPath,
        absPath,
        cwd: ctx.cwd,
      });
    ...
    renderCall: (args, theme) =>
      renderCall(
        args as { path: string; content?: string; providers?: string[] },
        theme
      ),

    renderResult: (result, options, theme) =>
      renderResult(
        result as AgentToolResult<GetDiagnosticsResult>,
        options,
        theme
      ),
  });
