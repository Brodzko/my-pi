...
describe('countBySeverity', () => {
  ...
  it('counts single severity', () => {
    ...
    expect(countBySeverity(diags)).toEqual({
      error: 2,
      warning: 0,
      info: 0,
      hint: 0,
...
describe('formatCounts', () => {
  it("returns '0 issues' when all counts are zero", () => {
    expect(formatCounts({ error: 0, warning: 0, info: 0, hint: 0 })).toBe(
      '0 issues'
    );
  });

  it('singularizes when count is 1', () => {
    expect(formatCounts({ error: 1, warning: 0, info: 0, hint: 0 })).toBe(
      '1 error'
    );
    expect(formatCounts({ error: 0, warning: 1, info: 0, hint: 0 })).toBe(
      '1 warning'
    );
  });

  it('pluralizes when count > 1', () => {
    expect(formatCounts({ error: 3, warning: 0, info: 0, hint: 0 })).toBe(
      '3 errors'
    );
  });

  it('joins multiple severities with commas', () => {
    expect(formatCounts({ error: 2, warning: 1, info: 0, hint: 0 })).toBe(
      '2 errors, 1 warning'
    );
  });
  ...
  it('omits zero-count severities', () => {
    expect(formatCounts({ error: 0, warning: 0, info: 5, hint: 0 })).toBe(
      '5 infos'
    );
  });
});
