...
const createMockProvider = (
  overrides: Partial<DiagnosticsProvider> = {}
): DiagnosticsProvider => ({
  id: 'mock',
...
describe('createDiagnosticsService', () => {
  ...
  describe('getDiagnostics', () => {
    ...
    it('defaults to typescript provider when providers not specified', async () => {
      const tsProvider = createMockProvider({
        id: 'typescript',
        getDiagnostics: vi.fn(async () => [
          makeDiagnostic({ provider: 'typescript' }),
        ]),
      });
    ...
    it('handles provider timeout gracefully (user-specified timeout)', async () => {
      const provider = createMockProvider({
        ...
        getDiagnostics: vi.fn(
          () =>
            new Promise<NormalizedDiagnostic[]>(resolve =>
    ...
    it('handles provider rejection gracefully', async () => {
      ...
      expect(result.providerStatus['typescript']?.message).toContain(
        'tsserver crashed'
      );
    });
  ...
  describe('cache', () => {
    ...
    it('does not use cache for multi-file requests', async () => {
      ...
      mockStat.mockReturnValue({ isDirectory: () => true } as ReturnType<
        typeof statSync
      >);
