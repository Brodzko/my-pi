...
const colorEntry = (entry: string, theme: Theme): string => {
  ...
  if (isDir) {
    return (
      theme.fg('syntaxVariable', icon) +
      ' ' +
      theme.bold(theme.fg('syntaxVariable', entry))
    );
  }
...
const layoutColumns = (items: string[], availableWidth: number): string[] => {
  ...
  for (let row = 0; row < numRows; row++) {
    ...
    for (let col = 0; col < numCols; col++) {
      ...
      const isLastCol =
        col === numCols - 1 || (col + 1) * numRows + row >= items.length;
...
export const createLsResultComponent = (
  ...
  if (!options.expanded) {
    // Collapsed: meta already in statusLine via config.meta, no extra content
    return undefined;
  }
  ...
  return {
    render(width: number): string[] {
      ...
      return statusLine ? [statusLine, '', ...grid] : grid;
    },
    invalidate() {},
  };
};
